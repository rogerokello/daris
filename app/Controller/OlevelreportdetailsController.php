...
*/
		...
		if ($this->request->is('post')) {
			...
			if(($this->request->data['enterspecificstudents'] == "1") && ($this->request->data['registrationnumbers'] != null)){
				...
				if($student_idr != false){
				    ...
				    $student_id_of_report = $this->Olevelreport->field('id',
	...
	if($mode == 3){
		...
		*/
		...
		$objPhpExcel->getActiveSheet()->setCellValue('C1','DARA CHRISTIAN HIGH SCHOOL');
		...
		if(count($report_subjects_considered) > 1){
			...
			foreach($report_exams as $anexamination){
			    ...
			    $amark = $this->Olevelmarksheetresult->field($asubject,
			...
			array_push($table,$report[$asubject],$grade_to_be_put,
			    $Gradeprofile->gradeprofile_returnsubjectremark($reports['Olevelreportdetail']['reportclass'], $report[$asubject])
